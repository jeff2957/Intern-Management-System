...
namespace Project2.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl, [Bind(Include = "UserId, UserName, UserMail")]AspNetUsers users)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    if (loginInfo.ExternalIdentity.GetUserName().Contains("AnYun") || loginInfo.ExternalIdentity.GetUserName().Contains("Ryuk"))
                    {                        
                        return RedirectToAction("AdminIndex", "Admin");
                    }
                    users.UserName = loginInfo.ExternalIdentity.GetUserName();
                ...
                default:
                    // 若使用者沒有帳戶，請提示使用者建立帳戶
                    users.Id = loginInfo.ExternalIdentity.GetUserId();
                    users.UserName = loginInfo.ExternalIdentity.GetUserName();
                    ...
                    var user = new ApplicationUser { UserName = loginInfo.ExternalIdentity.GetUserName(), Email = loginInfo.Email };
                    ...
                    if (result1.Succeeded)
                    {
                        ...
                        {
                            # region 角色權限
                            //if (model.IsAdminUser.ToString() == "Admin")
                            //{
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                case SignInStatus.Success:
                    HttpCookie ck = new HttpCookie("username", model.Email);
                    ck.HttpOnly = false;
                    Response.Cookies.Add(ck);
                    //var roleName = _db.AspNetRoles.Where(x => x.AspNetUsers.Any(u => u.UserName == model.Email)).Select(x => x.Name);
                    //var a = roleName.ToString();
                    //if (a == "Admin")
                    //{    
                    //    return RedirectToAction("AdminIndex", "Admin");     
                    //}
